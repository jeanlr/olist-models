...
mlflow.set_experiment(experiment_id='435850570539406161')
...
# Carregar a tabela tb_abt no pandas
abt_00 = pd.read_sql('SELECT * FROM tb_abt', conn)
...
abt_treino = abt_00[(abt_00['dt_ref'] >= '01-04-2017') & (abt_00['dt_ref'] <= '2018-03-01')]

# Filtrando a base de teste
abt_teste = abt_00[(abt_00['dt_ref'] >= '2018-04-01') & (abt_00['dt_ref'] <= '2018-06-01')]

# Definindo o nome da coluna target (ajuste conforme seu caso)
target_column = 'flag_churn_3m'  # substitua pelo nome real da sua coluna target

# Separando features (X) e target (y) na base de treino
X_train = abt_treino.drop(columns=[target_column, 'seller_id', 'dt_ref'])
y_train = abt_treino[target_column]

# Separando features (X) e target (y) na base de teste
X_test = abt_teste.drop(columns=[target_column, 'seller_id', 'dt_ref'])
y_test = abt_teste[target_column]

X_train.shape,X_test.shape
# %%
# Geração de metadados para análise das features
metadados = generate_metadata(df=X_train, targets=['TARGET', 'dt_ref', 'seller_id'], orderby='PC_NULOS')
metadados
# %%
...
tab['score'] = algoritmo.predict_proba(tab.drop(columns=['flag_churn_3m']))[:,0]  # Probabilidade de não evento
...
table = tab.groupby('decile').agg(
    ...
    event_rate=pd.NamedAgg(column='flag_churn_3m', aggfunc='mean'),  # Taxa de eventos (bad rate)
    volume=pd.NamedAgg(column='flag_churn_3m', aggfunc='size'),  # Número de observações
    qt_bads=pd.NamedAgg(column='flag_churn_3m', aggfunc=lambda x: (x == 1).sum()),  # Número de eventos
    perc_total_bads=pd.NamedAgg(column='flag_churn_3m', aggfunc=lambda x: (x == 1).sum()/tab[tab.flag_churn_3m == 1].shape[0])  # % do total de eventos
).reset_index()
...
tab['score'] = algoritmo.predict_proba(tab.drop(columns=['flag_churn_3m']))[:,0]
...
table = tab.groupby('decile').agg(
    min_score=pd.NamedAgg(column='score', aggfunc='min'),
    max_score=pd.NamedAgg(column='score', aggfunc='max'),
    event_rate=pd.NamedAgg(column='flag_churn_3m', aggfunc='mean'),
    volume=pd.NamedAgg(column='flag_churn_3m', aggfunc='size'),
    qt_bads=pd.NamedAgg(column='flag_churn_3m', aggfunc=lambda x: (x == 1).sum()),
    perc_total_bads=pd.NamedAgg(column='flag_churn_3m', aggfunc=lambda x: (x == 1).sum()/tab[tab.flag_churn_3m == 1].shape[0])
).reset_index()
