...
from (
    select 
        t2.seller_id,
        round(avg(t5.review_score), 2) as score_medio,
        round(max(t5.review_score), 2) as score_max,
        round(min(t5.review_score), 2) as score_min,
        t3.idade_dias_base,
        ...
        sum(t2.price) / count(distinct strftime('%m', t1.order_approved_at)) as tkt_med_venda_mes_ativado,
